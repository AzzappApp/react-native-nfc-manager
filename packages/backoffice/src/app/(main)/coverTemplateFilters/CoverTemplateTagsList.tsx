...
type Props = {
  search: string | null;
  count: number;
  coverTemplateTags: Array<CoverTemplateTag & { templates: number }>;
  page: number;
  pageSize: number;
...
const CoverTemplateTagsList = ({
  ...
  const updateSearchParams = useCallback(
    (
      ...
      startTransition(() => {
        router.replace(
          `/coverTemplateFilters?page=${page}&sort=${sort}&order=${order}&s=${search ?? ''}&status=${status ?? ''}`,
        );
      });
    },
