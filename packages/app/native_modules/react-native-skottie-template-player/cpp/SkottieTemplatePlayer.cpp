  ...
  if (propName == "render") {
    return jsi::Function::createFromHostFunction(
        ...
        [this](jsi::Runtime& runtime, const jsi::Value& thisValue,
          ...
          // android .asHostObject<RNSkia::JsiSkCanvas> is not working for some reason
...
TemplateResourceProvider::loadImageAsset(const char*, const char*,
                                         const char* resourceId) const {
  if (!assets.count(resourceId)) {
    return nullptr;
  }
  return assets.at(resourceId);
}

void TemplateResourceProvider::updateAssetImage(std::string id, sk_sp<SkImage> image, std::shared_ptr<SkMatrix> matrix) {

  auto asset = assets[id];
  if (asset == nullptr) {
    return;
  }
  asset->currentImage = image;
