...
class VignetteFilter: Filter<VignetteFilter.Parameters> {
  ...
  override fun draw(parameters: Parameters, glFramePool: GLFramePool, frameBufferPool: FrameBufferPool): GLFrame {
    ...
    val vignette = parameters.vignette
    ...
    val uResolution = glProgram.getUniformLocation("uResolution")
    GLES20.glUniform2fv(
      uResolution,
      1,
      floatArrayOf(inputImage.width.toFloat(),
      inputImage.height.toFloat()),
      0
    )

    val uReach = glProgram.getUniformLocation("offset")
    GLES20.glUniform1f(uReach, vignette / 2f)

    val uStrength = glProgram.getUniformLocation("darkness")
    GLES20.glUniform1f(uStrength, vignette)
    ...
    private val FRAGMENT_SHADER = """
     ...
     void main(){
       ...
       vec2 uv = vTexCoords.xy;
		   color *= smoothstep(0.8, offset * 0.799, distance(uv, vec2(0.5, 0.5)) * (darkness + offset));
