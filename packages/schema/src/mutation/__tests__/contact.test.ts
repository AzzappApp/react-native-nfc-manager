...
import {
  createContact as createContactData,
  updateContact,
...
describe('addContact Mutation', () => {
  ...
  test('should create a new contact if no existing contact found', async () => {
    ...
    (createContactData as jest.Mock).mockResolvedValue({ id: 'contact-123' });
    ...
    expect(createContactData).toHaveBeenCalled();
  ...
  test('should update a new contact if existing contact found', async () => {
    mockUser('user-456');
    (profileLoader.load as jest.Mock).mockResolvedValue(mockProfile);
    (webCardLoader.load as jest.Mock).mockResolvedValue(mockWebCard);
    (getContactByProfiles as jest.Mock).mockResolvedValue({
  ...
  test('should validate subscription if scan is used', async () => {
    ...
    (createContactData as jest.Mock).mockResolvedValue('contact-789');
