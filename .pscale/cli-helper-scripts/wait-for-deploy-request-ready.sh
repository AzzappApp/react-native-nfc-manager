function wait_for_deploy_request_ready {
    local retries=$1
    local db=$2
    local number=$3
    local org=$4
    ...
    while true; do
        ...
        if [ "$output" != "\"ready\"" ]; then
            ...
            echo  "Deploy-request $number is not deployed yet. Current status: $output"
            echo "Retrying in $wait seconds..."
            sleep $wait
        elif [ "$output" = "\"ready\"" ] ; then
            echo  "Deploy-request $number is ready to be deployed."
            return 0
        else
