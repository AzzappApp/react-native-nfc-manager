function create-db-branch {
    ...
    pscale branch create "$DB_NAME" "$BRANCH_NAME" --region eu-central --org "$ORG_NAME"
...
function create-db-branch-from {
    local DB_NAME=$1
    local BRANCH_NAME=$2
    local ORG_NAME=$3
...
function create-diff-for-ci {
    ...
    local deploy_request="https://app.planetscale.com/azzapp/azzapp/deploy-requests/${deploy_request_number}"
    ...
    echo "executing"
        ...
        BRANCH_DIFF="${BRANCH_DIFF//'\t'/' '}"
        # remove ' from BRANCH_DIFF
        BRANCH_DIFF=$(echo $BRANCH_DIFF | sed "s/'//g")
